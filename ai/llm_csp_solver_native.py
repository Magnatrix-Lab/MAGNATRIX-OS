"""CSP Solver - Constraint satisfaction for MAGNATRIX-OS."""
...
class CSPSolver:
    ...
    def stats(self) -> dict:
        return {"variables": len(self.variables), "constraints": len(self.constraints), "solution": self.solve() is not None}

def run():
    csp = CSPSolver()
    csp.add_variable("x", [1, 2, 3])
    csp.add_variable("y", [1, 2, 3])
    csp.add_constraint(Constraint(["x", "y"], lambda a: a.get("x") != a.get("y")))
    print("Solution:", csp.solve())
    print("Stats:", csp.stats())
